...
exports.savePostalCode = function(dbCollection, postalCode) {
    ...
    dbCollection.insertMany(postalCode,function(err,r) {
    test.equal(null, err);
    test.equal(154757, r.insertedCount);
...
exports.findByZipRadius = function (req, res, next) {
    ...
    findPostals(params, function(err, postals) {
      if (err) {
        ...
        console.log("ZIP radius search - Found %d postals", postals.length);
        res.send(postals);
      }
    });
};
...
exports.findByZipLimit = function (req, res, next) {
    ...
    findPostals(params, function(err, postals) {
      if (err) {
        ...
        console.log("ZIP radus limit search - Found %d postals", postals.length);
        res.send(postals);
      }
    });
};
  ...
  Step(
    ...
    function findNearCoords(err, postalObj) {
      ...
      /*var geoSearchParams = { 
        loc : { 
          $within: {
            $centerSphere : [postalObj.loc, milesToRadians(radiusMiles)], 
            $uniqueDocs:true
          }
        } 
      };
